...
class MemoryStore {
  ...
  addMemory(entry: Omit<MemoryEntry, "id" | "accessCount" | "createdAt" | "updatedAt" | "lastAccessedAt">): string {
    ...
    this.db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?)
    `, [id, this.sessionId, entry.type, entry.content, JSON.stringify(entry.metadata), entry.importance, now, now, now]);
  ...
  getMemory(id: string): MemoryEntry | null {
    ...
    // Update access count
    this.db.run(`UPDATE memories SET access_count = access_count + 1, last_accessed_at = ? WHERE id = ?`, [Date.now(), id]);
    this.save();
  ...
  getSessionContext(): SessionContext | null {
    ...
    const result = this.db.exec(`SELECT * FROM session_context WHERE session_id = ?`, [this.sessionId]);
    if (result.length === 0 || result[0].values.length === 0) return null;
  ...
  getStats(): { total: number; byType: Record<string, number>; avgImportance: number } {
    ...
    const totalResult = this.db.exec(`SELECT COUNT(*) as count FROM memories WHERE session_id = ?`, [this.sessionId]);
    const total = totalResult.length > 0 && totalResult[0].values.length > 0 ? totalResult[0].values[0][0] as number : 0;

    const byTypeResult = this.db.exec(`SELECT type, COUNT(*) as count FROM memories WHERE session_id = ? GROUP BY type`, [this.sessionId]);
    ...
    const avgResult = this.db.exec(`SELECT AVG(importance) as avg FROM memories WHERE session_id = ?`, [this.sessionId]);
...
const MemoryPlugin: Plugin = async (input: PluginInput): Promise<Hooks> => {
  ...
  // Use project path hash as session identifier for persistence
  const sessionId = `project_${Buffer.from(directory).toString("base64").slice(0, 16)}`;
  const store = new MemoryStore(dbPath, sessionId);
  
  await store.initialize();

  console.log(`[Memory] Initialized for session: ${sessionId}`);
      ...
      memory_query: tool({
        ...
        async execute(args) {
          ...
          const results = memories.map(m => 
            `- [${m.type}] ${m.content} (importance: ${m.importance.toFixed(2)})`
          );
